...
pub fn inner_merge_native<'local>(
    ...
    let (fragment, schema) = RT.block_on(async move {
        fragment
        .merge_columns(reader, left_on.as_str(), right_on.as_str(), jmax_field_id as i32)
        .await
    })?;
...
impl FromJObjectWithEnv<FileFragment> for JObject<'_> {
    fn extract_object(&self, env: &mut JNIEnv<'_>) -> Result<FileFragment> {
        let fragment_obj = env
            .call_method(self, "getFragment", "()Lcom/lancedb/lance/FragmentMetadata;", &[])?
            .l()?;
        ...
        Ok(FileFragment::new(Arc::new(dataset_guard.inner.clone()), fragment))
    }
}
